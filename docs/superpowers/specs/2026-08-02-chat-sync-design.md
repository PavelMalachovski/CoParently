...
**Correction (Task 5):** this section originally claimed the `messages` rules needed no change, on the theory that this design stops using per-message read state. That missed the migration paragraph above, which needs a *conversationId* update on an existing message document — a different field than `isRead`, and the old rule's `diff().affectedKeys().hasOnly(['isRead'])` denied it outright, permanently, for every message. The `messages` update rule now has a second, disjoint `hasOnly(['conversationId'])` branch gated by `canRepointMessage`: the destination must be *the canonical conversation* for the source conversation's participants (`canonicalConversationId`, the same sorted-join formula as `ConversationKey.of` in Kotlin) — never merely "a conversation both participants happen to share." An earlier version of this rule allowed the latter and was found, before it shipped, to let one parent create a look-alike conversation and re-point the other parent's message into it, hiding it from them permanently even though `messages` states `allow delete: if false`. The `isRead`-only path is unchanged and still cannot be combined with a `conversationId` change in the same write. See `firestore.rules` and `firestore-tests/rules/conversations-messages.test.js`.
